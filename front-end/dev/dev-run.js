...
portfinder.getPorts(2, {}, (portFinderErr, [docsPort, webpackPort]) => {
  ...
  runCmd('webpack-dev-server', `nodemon --exec webpack-dev-server -- --config webpack.docs.js --color --port ${webpackPort} --debug --hot --host ${ip.address()}`);

  runCmd('docs-server', 'nodemon --exec babel-node dev/server.js', {
    env: {
      PORT: docsPort,
