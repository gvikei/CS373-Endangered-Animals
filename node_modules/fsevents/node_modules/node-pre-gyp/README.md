...
You can use any of the methods described at http://public.aws.amazon.com/AWSJavaScriptSDK/guide/node-configuring.html.
...
Once you have committed an `appveyor.yml` ([appveyor.yml reference](http://www.appveyor.com/public/appveyor-yml)) to your GitHub repo and pushed it AppVeyor should automatically start building your project.
...
You might wish to publish binaries only on a specific commit. To do this you could borrow from the [Travis CI idea of commit keywords](http://about.travis-ci.org/public/user/how-to-skip-a-build/) and add special handling for commit messages with `[publish binary]`:
...
More details on Travis encryption at http://about.travis-ci.org/public/user/encryption-keys/.
...
If you want binaries for OS X in addition to linux you can enable [multi-os for Travis](http://public.travis-ci.com/user/multi-os/#Setting-.travis.yml)
...
Remember this publishing is not the same as `npm publish`. We're just talking about the binary module here and not your entire npm package. To automate the publishing of your entire package to npm on Travis see http://about.travis-ci.org/public/user/deployment/npm/
