...
#### Props
##### `to`
A [location descriptor](https://github.com/reactjs/react-router/blob/master/public/Glossary.md#locationdescriptor). Usually this is a string or an object, with the following semantics:
...
It also provides a `router` object on [context](https://facebook.github.io/react/public/context.html).
...
**Note**: Absolute paths may not be used in route config that is [dynamically loaded](/public/guides/DynamicRouting.md).
...
##### `children`
Routes can be nested, `this.props.children` will contain the element created from the child route component. Please refer to the [Route Configuration](/public/guides/RouteConfiguration.md) since this is a very critical part of the router's design.
...
##### `indexRoute`
The [index route](/public/guides/IndexRoutes.md). This is the same as specifying an `<IndexRoute>` child when using JSX route configs.
...
Note that the `<Redirect>` can be placed anywhere in the route hierarchy, though [normal precedence](/public/guides/RouteMatching.md#precedence) rules apply. If you'd prefer the redirects to be next to their respective routes, the `from` path will match the same as a regular route `path`.
...
Please see the [Index Routes guide](/public/guides/IndexRoutes.md).
...
#### `location`
The current [location](https://github.com/mjackson/history/blob/v2.x/public/Location.md).
...
#### `children`
The matched child route element to be rendered. If the route has [named components](/public/API.md#named-components) then this will be undefined, and the components will instead be available as direct properties on `this.props`.
...
For more details, please see the [histories guide](/public/guides/Histories.md).
...
[useQueries](https://github.com/ReactTraining/history/blob/master/public/QuerySupport.md)
and
[useBasename](https://github.com/ReactTraining/history/blob/master/public/BasenameSupport.md)
...
- `redirectLocation`: A [Location](/public/Glossary.md#location) object if the route is a redirect, `undefined` otherwise.
