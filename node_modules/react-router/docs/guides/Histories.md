...
### `browserHistory`
Browser history is the recommended history for browser application with React Router. It uses the [History](https://developer.mozilla.org/en-US/public/Web/API/History) API built into the browser to manipulate the URL, creating real URLs that look like `example.com/some/path`.
...
If you're using nginx, use the [`try_files` directive](http://nginx.org/en/public/http/ngx_http_core_module.html#try_files):
...
factory with the [useQueries](https://github.com/ReactTraining/history/blob/master/public/QuerySupport.md) and [useBasename](https://github.com/ReactTraining/history/blob/master/public/BasenameSupport.md) enhancers from `history`.
...
Using the
[useBeforeUnload](https://github.com/mjackson/history/blob/master/public/ConfirmingNavigation.md)
enhancer:
