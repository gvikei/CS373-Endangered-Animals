...
var publicRoot = _path2.default.join(repoRoot, configOptions.publicRoot || 'public-built/');
var tmpDocsRepo = _path2.default.join(repoRoot, configOptions.tmpDocsRepo || 'tmp-public-repo');
var publicRepo = configOptions.publicRepo; // if it is not set, then there is no public/site repo
var publicBuild = npmjson.scripts && npmjson.scripts['public-build'];
...
var versionBumpOptions = {
  type: argv._[0],
  preid: argv.onlyDocs ? 'public' : argv.preid,
...
if (!argv.skipVersionBumping && versionBumpOptions.type === undefined && versionBumpOptions.preid === undefined) {
  console.log('Must provide either a version bump type, "preid" or "--only-public"'.red);
...
function release(_ref) {
  ...
  if (npmjson.scripts) {
    ...
    // npm run build
    if (argv.onlyDocs && publicBuild) {
      console.log('Running: '.cyan + 'public-build'.green);
      runAndGitRevertOnError('npm run public-build');
      console.log('Completed: '.cyan + 'public-build'.green);
    } else {
  ...
  // documents site
  if (!isPrivate && publicRepo && (!preid || argv.onlyDocs)) {
    console.log('Releasing: '.cyan + 'documents site'.green);
    releaseAdRepo(publicRepo, publicRoot, tmpDocsRepo, vVersion);
